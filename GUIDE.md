...
- **管理知识库**：在终端运行 `bun run start` 使用 TUI 进行资料导入和摄入操作
...
```json
{
  "mcpServers": {
    "llmwiki": {
      "command": "bun",
      "args": ["run", "/Users/lichun/IdeaProjects/dragonfly-llmwiki/src/mcp-server.js"]
    }
  }
}
```
...
**Q: 我需要会编程吗？**
A: 不需要。安装好之后，只需要 `bun run start` 启动，然后跟 AI 对话就行。
